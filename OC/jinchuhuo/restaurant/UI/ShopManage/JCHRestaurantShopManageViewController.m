//
//  JCHRestaurantShopManageViewController.m
//  jinchuhuo
//
...
enum
{
    kAccountBookItem    = 0,// 账户
    kContactsItem       = 1,// 通讯录
    kDeviceItem         = 2,// 机具
    kEmptyPlaceholder   = 3,// 占位符
    kDishesItem         = 4,// 菜品
    kMaterialItem       = 5,// 原料
    kDeskItem           = 6,// 桌台
    kWarehouseItem      = 7,// 仓库
...
@interface JCHRestaurantShopManageViewController () <UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout, JCHShopAssistantTableViewCellDelegate, UIScrollViewDelegate, UITabBarControllerDelegate>
{
    UILabel *_titleLabel;
...
@implementation JCHRestaurantShopManageViewController
...
{
    NSArray *titleArray = @[@"",
                            @[@"仓库", @"通讯录", @"机具", @""], //@[@"账本", @"通讯录", @"仓库", loanText, @""]
                            @[@"菜品", @"原料", @"桌台", @"仓库设置"]];
    NSArray *iconArray  = @[@"",
                            @[@"mystore_icon_warehouse", @"mystore_icon_maillist", @"mystore_icon_copy", @""],
                            @[@"mystore_icon_dish",@"mystore_icon_material", @"mystore_icon_table", @"icon_takeout_mystore_whousesetting"],//@"mystore_icon_specifications"]
                            ];//@[@"mystore_icon_book", @"mystore_icon_maillist", @"mystore_icon_specifications", loanIcon, @""]
    ...
    if (indexPath.section == 0){
        ...
        cell.tag = (indexPath.section - 1) * 4 + indexPath.row;
        return cell;
    }
}
