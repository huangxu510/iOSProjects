//
//  JCHAddMaterialViewController.m
//  restaurant
//
...
{
    kProductWithoutSKU,     // 无规格原料
    kProductWithSKU,        // 多规格原料
...
@interface JCHAddMaterialViewController () <UITextViewDelegate,
...
UIAlertViewDelegate>
{
    ...
    JCHArrowTapView *supplierArrowTapView;
...
@property (retain, nonatomic, readwrite) ContactsRecord4Cocoa *currentContactRecord;
...
@implementation JCHAddMaterialViewController
...
- (id)initWithProductRecord:(ProductRecord4Cocoa *)productRecord
{
    ...
    if (self) {
        ...
        if (kJCHAddMaterialTypeAddProduct == self.productType) {
            self.title = @"添加原料";
        } else if (kJCHAddMaterialTypeModifyProduct == self.productType) {
            self.title = @"修改原料";
        }
        ...
        self.productModeTitles = @[@"无规格", @"多规格", @"主辅单位"];
...
- (void)createUI
{
    ...
    // 名称
    {
        ...
        nameTextView.placeholder = @"请输入原料名称";
    ...
    // 单位
    {
        ...
        unitArrowTapView.bottomLine.hidden = NO;
    ...
    // 供应商
    {
        supplierArrowTapView = [[[JCHArrowTapView alloc] initWithFrame:CGRectZero] autorelease];
        supplierArrowTapView.titleLabel.text = @"供应商";
        supplierArrowTapView.titleLabel.font = [UIFont systemFontOfSize:15.0];
        supplierArrowTapView.bottomLine.hidden = NO;
        [supplierArrowTapView.button addTarget:self action:@selector(selectSupplier) forControlEvents:UIControlEventTouchUpInside];
        [self.backgroundScrollView addSubview:supplierArrowTapView];
        
        [supplierArrowTapView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.equalTo(self.backgroundScrollView);
            make.width.mas_equalTo(kScreenWidth);
            make.top.equalTo(categoryArrowTapView.mas_bottom);
            make.height.mas_equalTo(kStandardItemHeight);
    ...
    //商家简称
    {
        traderCodeTitleLabel = [JCHUIFactory createLabel:CGRectZero title:@"供应商"
                                                    font:textFont
                                               textColor:textColor
                                                  aligin:NSTextAlignmentLeft];
        ...
        NSAttributedString *placeHolder = [[[NSAttributedString alloc] initWithString:@"请输入供应商信息" attributes:@{NSForegroundColorAttributeName : UIColorFromRGB(0xd5d5d5), NSFontAttributeName : textFont}] autorelease];
    ...
    // 提示信息
    {
        ...
        [seperatorView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(traderCodeTextField.mas_bottom);
            make.left.equalTo(self.backgroundScrollView.mas_left);
            make.width.mas_equalTo(kScreenWidth);
    ...
    // 原料模式选择
    {
        ...
        skuModeProductLabel.titleLabel.text = @"原料属性";
    ...
    // 有规格原料容器
    {
        ...
        withSKUInventoryView = [[[JCHArrowTapView alloc] initWithFrame:CGRectZero] autorelease];
        [withSKUContainerView addSubview:withSKUInventoryView];
        withSKUInventoryView.titleLabel.text = @"•  单品期初库存";
        withSKUInventoryView.titleLabel.font = textFont;
        withSKUInventoryView.titleLabel.textColor = textColor;
        [withSKUInventoryView.button addTarget:self
                                        action:@selector(handleSetSpecificationInventory:)
                              forControlEvents:UIControlEventTouchUpInside];
        [withSKUInventoryView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.and.right.equalTo(withSKUContainerView);
        ...
        [withSKUBarCodeView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.and.right.equalTo(withSKUContainerView);
            make.top.equalTo(withSKUInventoryView.mas_bottom);
            make.height.mas_equalTo(kStandardItemHeight);
        }];
    }
    ...
    // 主辅助单位容器
    {
        ...
        mainUnitInventoryView = [[[JCHArrowTapView alloc] initWithFrame:CGRectZero] autorelease];
        [mainUnitContainerView addSubview:mainUnitInventoryView];
        mainUnitInventoryView.titleLabel.text = @"•  主辅原料期初库存";
        mainUnitInventoryView.titleLabel.font = textFont;
        mainUnitInventoryView.titleLabel.textColor = textColor;
        [mainUnitInventoryView.button addTarget:self
                                         action:@selector(handleSetMainAuxilaryUnitInventory:)
                               forControlEvents:UIControlEventTouchUpInside];
        [mainUnitInventoryView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.and.right.equalTo(mainUnitContainerView);
        ...
        mainUnitBarCodeView.titleLabel.text = @"•  主辅原料条码";
        ...
        [mainUnitBarCodeView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.and.right.equalTo(mainUnitContainerView);
            make.top.equalTo(mainUnitInventoryView.mas_bottom);
            make.height.mas_equalTo(kStandardItemHeight);
        }];
    }
    ...
    // 无规格原料容器
    {
        ...
        withoutSKUInventoryView = [[[JCHArrowTapView alloc] initWithFrame:CGRectZero] autorelease];
        [withoutSKUContainerView addSubview:withoutSKUInventoryView];
        withoutSKUInventoryView.titleLabel.text = @"•  期初库存";
        withoutSKUInventoryView.titleLabel.font = textFont;
        withoutSKUInventoryView.titleLabel.textColor = textColor;
        [withoutSKUInventoryView.button addTarget:self
                                           action:@selector(handleSetWithoutSKUInventory:)
                                 forControlEvents:UIControlEventTouchUpInside];
        [withoutSKUInventoryView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.and.right.equalTo(mainUnitContainerView);
        ...
        withoutSKUBarCodeView = [[[JCHTitleTextField alloc] initWithTitle:@"•  原料条码"
                                                                     font:textFont
        ...
        [withoutSKUBarCodeView mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.top.equalTo(withoutSKUInventoryView.mas_bottom);
            make.height.mas_equalTo(kStandardItemHeight);
        }];
    }
    ...
    //原料启用
    {
        ...
        productStatusView.titleLabel.text = @"原料启用";
...
- (void)loadData
{
    ...
    else //修改原料
    {
        ...
        //如果是修改原料，switch的状态对应productRecord的记录
        productStatusView.switchButton.on = !self.productRecord.goods_hiden_flag;
...
#if 0
    if (kProductWithoutSKU == currentProductMode) {
        self.skuInventoryArray = [[[NSMutableArray alloc] init] autorelease];
        self.unitInventoryArray = [self createDefaultUnitInventory:self.unitInventoryArray];
    } else if (kProductWithSKU == currentProductMode) {
        self.skuInventoryArray = [[[NSMutableArray alloc] init] autorelease];
        self.noskuInventoryArray = [self createDefaultNoSKUInventory:self.noskuInventoryArray];
        ...
        
        {
            ...
            if (currentProductMode == kProductWithSKU) {
                ...
                for (JCHBeginInventoryForSKUViewData *data in self.skuInventoryArray) {
                    ...
                    record.skuUUIDVector = data.skuUUIDVector;
                    record.beginningPrice = data.price;
                    record.beginningCount = data.count;
                    record.warehouseUUID = data.warehouseUUID;
                    record.unitUUID = self.currentUnitRecord.unitUUID;
                    record.unitName = self.currentUnitRecord.unitName;
                    [beginInventoryRecordsArray addObject:record];
                }
            } else if (currentProductMode == kProductMainUnit){
                // 主辅单位期初库存
                for (JCHBeginInventoryForSKUViewData *data in self.unitInventoryArray) {
                    BeginningInventoryInfoRecord4Cocoa *record = [[[BeginningInventoryInfoRecord4Cocoa alloc] init] autorelease];
                    record.skuUUIDVector = @[];
                    record.beginningPrice = data.price;
                    record.beginningCount = data.count;
                    record.unitUUID = data.unitUUID;
                    record.unitName = data.unit;
                    record.warehouseUUID = data.warehouseUUID;
                    [beginInventoryRecordsArray addObject:record];
                }
            } else {
                // 无规格期初库存
                for (JCHBeginInventoryForSKUViewData *data in self.noskuInventoryArray) {
                    BeginningInventoryInfoRecord4Cocoa *record = [[[BeginningInventoryInfoRecord4Cocoa alloc] init] autorelease];
                    record.skuUUIDVector = @[];
                    record.beginningPrice = data.price;
                    record.beginningCount = data.count;
            ...
            if (currentProductMode == kProductWithSKU) {
                ...
                //先将之前的productItemRecord删除掉(当原料的skuTypeList改变的时候)
                NSMutableArray *skuTypeListBefore = [NSMutableArray array];
                NSMutableArray *skuTypeListAfter = [NSMutableArray array];
                for (NSDictionary *dict in skuRecord.skuArray) {
                    SKUValueRecord4Cocoa *skuValueRecord = [[[dict allValues] firstObject] firstObject];
                    [skuTypeListBefore addObject:skuValueRecord.skuTypeUUID];
                }
                
                for (NSDictionary *dict in self.currentSKURecord.skuArray) {
                    SKUValueRecord4Cocoa *skuValueRecord = [[[dict allValues] firstObject] firstObject];
                    [skuTypeListAfter addObject:skuValueRecord.skuTypeUUID];
                }
                if (![JCHTransactionUtility skuUUIDs:skuTypeListBefore isEqualToArray:skuTypeListAfter]) {
                    NSArray<ProductItemRecord4Cocoa *> *productItemList = [productService querySkuGoodsItem:productUUID];
                    for (ProductItemRecord4Cocoa *productItemRecord in productItemList) {
                        [productService deleteGoodsItem:productRecord.goods_uuid unitUUID:productItemRecord.goodsUnitUUID skuUUID:productItemRecord.goodsSkuUUID];
                    }
                }
                ...
                for (ProductSalePriceRecord *record in self.skuSalePriceArray) {
                    ...
                    price.goodsSkuUUID = @"";
                    price.goodsUnitUUID = @"";
                    price.skuUUIDVector = record.recordUUID;
                    ...
                    price.itemPrice = record.productPrice.doubleValue;
                    
                    for (ProductBarCodeRecord *barCode in self.skuBarCodeArray) {
                        if (YES == [JCHTransactionUtility skuUUIDs:price.skuUUIDVector isEqualToArray:barCode.recordUUID]) {
                            price.itemBarCode = barCode.productBarCode;
                            break;
                        }
                    }
                    
                    for (JCHBeginInventoryForSKUViewData *inventory in self.skuInventoryArray) {
                        if (YES == [JCHTransactionUtility skuUUIDs:inventory.skuUUIDVector isEqualToArray:record.recordUUID]) {
                            price.goodsSkuUUID = inventory.goodsSKUUUID;;
                            break;
                        }
                    }
                ...
                // 删除之前的主辅助单位记录
                [productService deleteGoodsUnit:productUUID];
                
                // 添加主辅单位
                for (ProductUnitRecord *record in self.unitRecordArray) {
                    ProductUnitRecord4Cocoa *unit = [[[ProductUnitRecord4Cocoa alloc] init] autorelease];
                    unit.goodsUUID = productUUID;
                    unit.unitUUID = record.unitRecord.unitUUID;
                    unit.ratio = record.convertRatio;
                    [productService addGoodsUnit:unit];
                }
                
                NSArray *allUnitList = [[[ServiceFactory sharedInstance] unitService] queryAllUnit];
                for (ProductSalePriceRecord *record in self.unitSalePriceArray) {
                    ProductItemRecord4Cocoa *price = [[[ProductItemRecord4Cocoa alloc] init] autorelease];
                    price.goodsUUID = productUUID;
                    price.goodsSkuUUID = @"";
                    ...
                    for (ProductBarCodeRecord *barCode in self.unitBarCodeArray) {
                        if ([barCode.productName isEqualToString:record.recordUUID]) {
                            price.itemBarCode = barCode.productBarCode;
                            break;
                        }
                    }
                    ...
                    for (UnitRecord4Cocoa *unit in allUnitList) {
                        if ([unit.unitName isEqualToString:record.recordUUID]) {
                            price.goodsUnitUUID = unit.unitUUID;
                            break;
                        }
                    }
            ...
            if (self.productType == kJCHProductTypeModifyProduct)
            {
                ...
                // 如果修改了原料名称，分类，检查修改后的名称与分类是否已存在于数据库中
                ...
                if ([productService updateProduct:productRecord recordVector:beginInventoryRecordsArray])
                {
                    [MBProgressHUD hideAllHudsForWindow];
                    [self showAlertView:@"修改原料失败"];
                } else {
                    [MBProgressHUD hideAllHudsForWindow];
                }
            }
            else
            {
                ...
                NSMutableArray *validInventoryArray = [[[NSMutableArray alloc] init] autorelease];
                
                for (BeginningInventoryInfoRecord4Cocoa *record in beginInventoryRecordsArray) {
                    if (record.beginningCount <= 0.0000001) {
                        continue;
                    }
                    
                    [validInventoryArray addObject:record];
                }
                
                
                
                TICK;
                int status = [productService addProduct:productRecord recordVector:validInventoryArray];
                TOCK(@"addProduct");
                if (status) {
                    [MBProgressHUD hideAllHudsForWindow];
                    [self showAlertView:@"添加原料失败"];
                } else {
                    [MBProgressHUD hideAllHudsForWindow];
                }
            }
...
{
    WeakSelf;
    JCHRestaurantCategoryViewController *categoryListViewController = [[[JCHRestaurantCategoryViewController alloc] initWithType:kJCHRestaurantCategoryListTypeSelect] autorelease];
...
{
    WeakSelf;
    JCHGroupContactsViewController *contactsViewController = [[[JCHGroupContactsViewController alloc] initWithType:kJCHGroupContactsSupplier selectMember:YES] autorelease];
    contactsViewController.sendValueBlock = ^(ContactsRecord4Cocoa *selectContactRecord){
        weakSelf.currentContactRecord = selectContactRecord;
        weakSelf -> supplierArrowTapView.detailLabel.text = selectContactRecord.name;
    };
    UINavigationController *navController = [[[UINavigationController alloc] initWithRootViewController:contactsViewController] autorelease];
...
- (void)handleSetSpecificationInventory:(id)sender
{
    if (self.skuInventoryArray.count == 0) {
        [self showAlertView:@"请先添加原料规格，然后设置原料规格对应的期初库存"];
        return;
    }
...
- (void)handleSetSKUBarCode:(id)sender
{
    if (self.skuBarCodeArray.count == 0) {
        [self showAlertView:@"请先添加原料规格，然后设置原料规格对应的条码"];
        return;
    }
...
- (void)handleSetSKUSalePrice:(id)sender
{
    if (self.skuSalePriceArray.count == 0) {
        [self showAlertView:@"请先添加原料规格，然后设置原料规格对应的售价"];
        return;
    }
...
{
    NSMutableArray *tempSkuInventoryArray = [NSMutableArray arrayWithArray:self.skuInventoryArray];
    NSMutableArray *tempSkuBarCodeArray = [NSMutableArray arrayWithArray:self.skuBarCodeArray];
    NSMutableArray *tempSkuSalePriceArray = [NSMutableArray arrayWithArray:self.skuSalePriceArray];
    ...
    for (NSInteger i = 0; i < skuValueCombineNum; i++) {
        for (WarehouseRecord4Cocoa *warehouse in warehouseList) {
            ...
            for (JCHBeginInventoryForSKUViewData *inventory in tempSkuInventoryArray) {
    ...
    for (NSInteger i = 0; i < skuValueCombineNum; i++) {
        ...
        if (NO == findSame) {
            ProductSalePriceRecord *price = [[[ProductSalePriceRecord alloc] init] autorelease];
            price.recordUUID = skuValueUUIDsArray[i];
            price.productPrice = @"0.00";
            price.productName = skuValueCombineArray[i];
            [tempSkuSalePriceArray addObject:price];
        }
    }
