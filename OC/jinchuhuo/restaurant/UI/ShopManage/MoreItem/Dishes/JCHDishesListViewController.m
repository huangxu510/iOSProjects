...
@interface JCHDishesListViewController () <JCHItemListFooterViewDelegate,
...
@implementation JCHDishesListViewController
...
- (void)handleAddMode:(id)sender
{
    ...
    JCHAddDishesViewController *addController = [[[JCHAddDishesViewController alloc] initWithProductRecord:nil] autorelease];
    [self.navigationController pushViewController:addController animated:YES];
#else
    JCHTakeoutAddDishesViewController *addController = [[[JCHTakeoutAddDishesViewController alloc] initWithProductRecord:nil] autorelease];
    [self.navigationController pushViewController:addController animated:YES];
#endif
#if 0
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    WeakSelf;
    cell.putawayBlock = ^{
        [weakSelf handlePutaway:indexPath];
    };
    cell.soldoutBlock = ^{
        [weakSelf handleSoldOut:indexPath];
    };
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    else
    {
        ...
        id<ProductService> productService = [[ServiceFactory sharedInstance] productService];
        ProductRecord4Cocoa *productRecord = [productService queryCuisine:[(ProductRecord4Cocoa *)(self.productRecordArray[indexPath.section][indexPath.row]) goods_uuid]];
        
        
#if MMR_RESTAURANT_VERSION
        JCHAddDishesViewController *modifyRecordVC = [[[JCHAddDishesViewController alloc] initWithProductRecord:productRecord] autorelease];
        
        modifyRecordVC.productType = kJCHAddDishesTypeModifyProduct;
        appearFromPop = YES;
        [self.navigationController pushViewController:modifyRecordVC animated:YES];
#else
        JCHTakeoutAddDishesViewController *modifyRecordVC = [[[JCHTakeoutAddDishesViewController alloc] initWithProductRecord:productRecord] autorelease];
...
#pragma mark - Takeout 上架
- (void)handlePutaway:(NSIndexPath *)indexPath
{
    ProductRecord4Cocoa *productRecord = self.productRecordArray[indexPath.section][indexPath.row];
    
    if (productRecord.is_multi_unit_enable) {
        UIAlertView *alertView = [[[UIAlertView alloc] initWithTitle:@"温馨提示"
                                                             message:@"主副单位商品不能上架，请修改商品属性"
                                                            delegate:nil
        ...
        LCActionSheet *actionSheet = [LCActionSheet sheetWithTitle:@"选择上架的平台"
                                                      buttonTitles:@[@"美团外卖", @"饿了么", @"百度外卖"]
                                                    redButtonIndex:-1
                                                           clicked:^(NSInteger buttonIndex){
                                                               JCHTakeoutResource takeoutResource = buttonIndex + 1;
                                                               JCHTakeoutPutawayViewController *putawayVC = [[[JCHTakeoutPutawayViewController alloc] initWithProductRecord:productRecord takeoutResource:takeoutResource] autorelease];
                                                               [self.navigationController pushViewController:putawayVC animated:YES];
                                                           }];
        [actionSheet show];
    }
}
...
#pragma mark - Takeout 下架
- (void)handleSoldOut:(NSIndexPath *)indexPath
{
    LCActionSheet *actionSheet = [LCActionSheet sheetWithTitle:@"选择下架的平台"
                                                  buttonTitles:@[@"美团外卖", @"饿了么", @"百度外卖"]
                                                redButtonIndex:-1
                                                       clicked:^(NSInteger buttonIndex){
                                                           JCHTakeoutResource takeoutResource = buttonIndex + 1;
                                                           [self handleSoldOut:indexPath takeoutResource:takeoutResource];
                                                       }];
    [actionSheet show];
}

- (void)handleSoldOut:(NSIndexPath *)indexPath takeoutResource:(JCHTakeoutResource)takeoutResource
{
    ProductRecord4Cocoa *productRecord = self.productRecordArray[indexPath.section][indexPath.row];
    JCHSyncStatusManager *statusManager = [JCHSyncStatusManager shareInstance];
    
    id<TakeOutService> takeoutService = [[ServiceFactory sharedInstance] takeoutService];
    SoldOutProductRequest *request = [[[SoldOutProductRequest alloc] init] autorelease];
    request.token = statusManager.syncToken;
    request.bookID = statusManager.accountBookID;
    request.serviceURL = [NSString stringWithFormat:@"%@/dish/putOff", kTakeoutServerIP];
    
    NSString *takeoutBindID = [JCHTakeoutManager getDishTakeoutBindID:productRecord takeoutResource:takeoutResource];
    
    if ([takeoutBindID isEmptyString]) {
        UIAlertView *alertView = [[[UIAlertView alloc] initWithTitle:@"温馨提示" message:@"该菜品在该平台尚未上架" delegate:nil cancelButtonTitle:@"我知道了" otherButtonTitles:nil] autorelease];
        [alertView show];
        return;
    }
    ...
    [MBProgressHUD showHUDWithTitle:@"下架中..." detail:@"" duration:9999 mode:MBProgressHUDModeIndeterminate superView:self.view completion:nil];
    [takeoutService soldOutProduct:request callback:^(id response) {
        ...
        if (kJCHServiceSuccess == [data[@"status"] integerValue]) {
            ...
            if (NO == [responseStatus isEqualToString:kJCHServiceResponseSuccess]) {
                ...
                [MBProgressHUD showHUDWithTitle:@"成功下架"
                                         detail:@""
                                       duration:kJCHDefaultShowHudTime
                                           mode:MBProgressHUDModeText
                                      superView:self.view
                                     completion:nil];
                [JCHTakeoutManager updateTakeoutStatus:0 takeoutResource:takeoutResource dish:productRecord];
                id<ProductService> productService = [[ServiceFactory sharedInstance] productService];
        ...
        if (0 != canNotDeleteProductList.count) {
            ...
            UIAlertAction *deleteAction = [UIAlertAction actionWithTitle:@"强制删除"
                                                                 ...
                                                                 handler:^(UIAlertAction * _Nonnull action) {
                                                                     [self deleteAllSelectedProduct:selectedIndexPaths clearProductInventory:YES];
                                                                 }];
            ...
            [self deleteAllSelectedProduct:selectedIndexPaths clearProductInventory:NO];
        }
    });
}
...
- (void)handleDeleteProduct:(NSIndexPath *)indexPath
{
    ...
    if (NO == showAlertForDeleteProduct) {
        ...
        UIAlertAction *deleteAction = [UIAlertAction actionWithTitle:@"强制删除"
                                                               style:UIAlertActionStyleDestructive
                                                             handler:^(UIAlertAction * _Nonnull action) {
                                                                 [self deleteAllSelectedProduct:@[indexPath] clearProductInventory:YES];
                                                             }];
        ...
        [self deleteAllSelectedProduct:@[indexPath] clearProductInventory:NO];
    }
...
- (void)loadProductData
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        self.allProductRecord = [productService queryAllCuisine:YES];
...
- (void)deleteAllSelectedProduct:(NSArray *)selectedIndexPaths clearProductInventory:(BOOL)clearProductInventory
{
#if MMR_TAKEOUT_VERSION
    [self takeoutDeleteProductRecordList:selectedIndexPaths clearProductInventory:clearProductInventory];
#else
    [MBProgressHUD showHUDWithTitle:@"删除中..." detail:@"" duration:9999 mode:MBProgressHUDModeIndeterminate completion:nil];
...
#pragma mark - 
#pragma mark 外卖版删除商品
- (void)takeoutDeleteProductRecordList:(NSArray *)indexPaths clearProductInventory:(BOOL)clearProductInventory
{
    NSMutableArray *meituanDishIDList = [NSMutableArray array];
    NSMutableArray *elemeDishIDList = [NSMutableArray array];
    NSMutableArray *baiduDishIDList = [NSMutableArray array];
    
    for (NSIndexPath *indexPath in indexPaths) {
        ProductRecord4Cocoa *currentProduct = self.productRecordArray[indexPath.section][indexPath.row];
        if (currentProduct.takoutRecord.meituanBindID && ![currentProduct.takoutRecord.meituanBindID isEmptyString]) {
            [meituanDishIDList addObject:currentProduct.takoutRecord.meituanBindID];
        }
        
        if (currentProduct.takoutRecord.eleBindID && ![currentProduct.takoutRecord.eleBindID isEmptyString]) {
            [elemeDishIDList addObject:currentProduct.takoutRecord.eleBindID];
        }
        
        if (currentProduct.takoutRecord.baiduBindID && ![currentProduct.takoutRecord.baiduBindID isEmptyString]) {
            [baiduDishIDList addObject:currentProduct.takoutRecord.baiduBindID];
        }
    }
    
    
    NSDictionary *dishIDDict = @{@(kJCHTakeoutResourceMeituan).stringValue : meituanDishIDList,
                                 @(kJCHTakeoutResourceEleme).stringValue : elemeDishIDList,
                                 @(kJCHTakeoutResourceBaidu).stringValue : baiduDishIDList};
    
    JCHSyncStatusManager *statusManager = [JCHSyncStatusManager shareInstance];
    
    id<TakeOutService> takeoutService = [[ServiceFactory sharedInstance] takeoutService];
    DeleteProductRequest *request = [[[DeleteProductRequest alloc] init] autorelease];
    request.token = statusManager.syncToken;
    request.bookID = statusManager.accountBookID;
    request.serviceURL = [NSString stringWithFormat:@"%@/dish/delete", kTakeoutServerIP];
    request.dishList = dishIDDict;
    
    if (indexPaths.count == 1) {
        [MBProgressHUD showHUDWithTitle:@"删除中..." detail:@"" duration:9999 mode:MBProgressHUDModeIndeterminate completion:nil];
    }
    
    [takeoutService deleteProduct:request callback:^(id response) {
