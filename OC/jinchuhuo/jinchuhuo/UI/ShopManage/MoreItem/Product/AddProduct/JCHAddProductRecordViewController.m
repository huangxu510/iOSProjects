//
//  JCHAddProductRecordViewController.m
//  jinchuhuo
//
...
{
    kProductWithoutSKU,     // 无规格商品
    kProductWithSKU,        // 多规格商品
...
@interface JCHAddProductRecordViewController () <UITextViewDelegate,
...
@implementation JCHAddProductRecordViewController
...
- (void)createUI
{
    ...
    // 商品模式选择
    {
        ...
        skuModeProductLabel.titleLabel.text = @"商品属性";
    ...
    // 有规格商品容器
    {
        ...
        if (currentProductMode == kProductWithSKU) {
            containerViewHeight = 4 * kStandardItemHeight;
        }
    ...
    // 主辅助单位容器
    {
        ...
        mainUnitPriceView.titleLabel.text = @"•  主辅商品售价";
        ...
        mainUnitInventoryView.titleLabel.text = @"•  主辅商品期初库存";
        ...
        mainUnitBarCodeView.titleLabel.text = @"•  主辅商品条码";
    ...
    // 无规格商品容器
    {
        ...
        withoutSKUPriceView = [[[JCHTitleTextField alloc] initWithTitle:@"•  商品售价"
                                                                   font:textFont
                                                            placeholder:@"0.00"
                                                              textColor:textColor
        ...
        withoutSKUBarCodeView = [[[JCHTitleTextField alloc] initWithTitle:@"•  商品条码"
                                                                     font:textFont
    ...
    //商品启用
    {
        ...
        productStatusView.titleLabel.text = @"商品启用";
...
- (void)restoreUIStatus
{
    ...
    for (size_t i = 0; i < sizeof(cameraButtonArray) / sizeof(cameraButtonArray[0]); ++i) {
        ...
        if (nil != imageName &&
            ...
            UIImage *buttonImage = [UIImage jchProductImageNamed:imageName];
            if (nil == buttonImage) {
...
- (void)loadData
{
    ...
    else //修改商品
    {
        ...
        //如果是修改商品，switch的状态对应productRecord的记录
        productStatusView.switchButton.on = !self.productRecord.goods_hiden_flag;
...
- (void)handleSaveRecord:(id)sender
{
    ...
    if (nameTextView.text == nil ||
        ...
        [self showAlertView:@"请输入您添加的商品名称"];
        return;
    }
    ...
    
    {
        if (self.productType == kJCHProductTypeAddProduct) {
            ...
            for (ProductRecord4Cocoa *product in allProduct) {
                if ([product.goods_name isEqualToString:nameTextView.text] && [product.goods_type isEqualToString:categoryArrowTapView.detailLabel.text]) {
                    [self showAlertView:@"该商品已存在"];
                    return;
                }
            }
        }
    ...
    if (currentProductMode == kProductWithSKU) {
        ...
        if (YES == showAlertFlag) {
            [self showAlertView:@"填写多规格商品的单品销售价格，出货时更方便"];
            return;
        }
    }
    ...
    if (currentProductMode == kProductMainUnit) {
        ...
        if (YES == showAlertFlag) {
            [self showAlertView:@"填写商品的主辅单位销售价格，出货时更方便"];
            return;
        }
        
        if (0 == self.unitRecordArray.count) {
            [self showAlertView:@"请设置商品的辅单位"];
            return;
        }
    }
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.01 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        
        {
            ...
            if (currentProductMode == kProductWithSKU) {
                ...
                //先将之前的productItemRecord删除掉(当商品的skuTypeList改变的时候)
            ...
            if (self.productType == kJCHProductTypeModifyProduct)
            {
                ...
                // 如果修改了商品名称，分类，检查修改后的名称与分类是否已存在于数据库中
                ...
                int status = [productService updateProduct:productRecord recordVector:beginInventoryRecordsArray];
                
                if (status)
                {
                    [MBProgressHUD hideAllHudsForWindow];
                    [self showAlertView:[NSString stringWithFormat:@"修改商品失败, 错误码 %d", status]];
                } else {
                    [MBProgressHUD hideAllHudsForWindow];
                }
            }
            else
            {
                ...
                if (status) {
                    [MBProgressHUD hideAllHudsForWindow];
                    [self showAlertView:[NSString stringWithFormat:@"添加商品失败, 错误码 %d", status]];
                } else {
                    [MBProgressHUD hideAllHudsForWindow];
                }
            }
...
- (void)handleSetSpecificationInventory:(id)sender
{
    if (self.skuInventoryArray.count == 0) {
        [self showAlertView:@"请先添加商品规格，然后设置商品规格对应的期初库存"];
        return;
    }
...
- (void)handleSetSKUBarCode:(id)sender
{
    if (self.skuBarCodeArray.count == 0) {
        [self showAlertView:@"请先添加商品规格，然后设置商品规格对应的条码"];
        return;
    }
...
- (void)handleSetSKUSalePrice:(id)sender
{
    if (self.skuSalePriceArray.count == 0) {
        [self showAlertView:@"请先添加商品规格，然后设置商品规格对应的售价"];
        return;
    }
...
- (void)layoutUnitSKUContainerView:(enum ProductUnitSKUMode)enumMode
{
    switch (enumMode) {
        ...
        case kProductWithSKU: {
            ...
            [withSKUContainerView mas_updateConstraints:^(MASConstraintMaker *make) {
                make.height.mas_equalTo(4 * kStandardItemHeight);
            }];
...
- (void)refreshSKUPriceBarCodeInventory
{
    ...
    for (NSInteger i = 0; i < skuValueCombineNum; i++) {
        for (WarehouseRecord4Cocoa *warehouse in warehouseList) {
            ...
            JCHBeginInventoryForSKUViewData *findInventoryData = nil;
            for (JCHBeginInventoryForSKUViewData *inventory in self.skuInventoryArray) {
            ...
            if (NO == findInventoryForWarehouse) {
                ...
                [tempSkuInventoryArray addObject:findInventoryData];
            }
        }
    }
