...
- (MXPullDownMenu *)initWithArray:(NSArray *)array selectedColor:(UIColor *)color titleLabelHeight:(CGFloat)height
{
    self = [super init];
    if (self) {
...
//        CGFloat textLayerInterval = self.frame.size.width / ( _numOfMenu * 2);
        ...
        for (int i = 0; i < _numOfMenu; i++) {
            ...
            CGPoint position = CGPointMake(10, self.frame.size.height / 2);
...
- (void)layoutSubviews
{
    ...
    if (_index == 0)
    {
        CALayer *indicator = [self creatIndicatorWithColor:_menuColor andPosition:CGPointMake(self.frame.size.width - 20, self.frame.size.height / 2)];
...
{
    JCHPullDownMenuTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"cell"];
    if (!cell) {
        cell = [[JCHPullDownMenuTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"cell"];
    }
    
    [cell setAccessoryType:UITableViewCellAccessoryNone];
    [cell setNameLabel:_array[_currentSelectedMenudIndex][indexPath.row]];
    cell.selectionStyle = UITableViewCellSelectionStyleNone;

    if ([cell nameLabelText] == [(CATextLayer *)[_titles objectAtIndex:_currentSelectedMenudIndex] string]) {
//        [cell setAccessoryType:UITableViewCellAccessoryCheckmark];
        [cell setCheckmarkImageViewSelected:YES];
    }
    else
    {
        [cell setCheckmarkImageViewSelected:NO];
    }
...
- (CALayer *)creatIndicatorWithColor:(UIColor *)color andPosition:(CGPoint)point
{
...
//    layer.fillColor = color.CGColor;
    ...
    layer.contents = (id)[UIImage imageNamed:@"tip_down"].CGImage;
    
//    CGPathRef bound = CGPathCreateCopyByStrokingPath(layer.path, nil, layer.lineWidth, kCGLineCapButt, kCGLineJoinMiter, layer.miterLimit);
//    layer.bounds = CGPathGetBoundingBox(bound);
    layer.bounds = CGRectMake(0, 0, 16, 8.5);
...
- (CATextLayer *)creatTextLayerWithNSString:(NSString *)string withColor:(UIColor *)color andPosition:(CGPoint)point
{
    ...
    layer.string = string;
    layer.fontSize = [JCHSizeUtility calculateHeightWithNavigationBar:YES TabBar:NO sourceHeight:16.0f];
...
- (UITableView *)creatTableViewAtPosition:(CGPoint)point
{
    ...
    tableView.rowHeight = [JCHSizeUtility calculateHeightWithNavigationBar:YES TabBar:NO sourceHeight:50.0f];
...
{
    CGFloat fontSize = [JCHSizeUtility calculateHeightWithNavigationBar:YES TabBar:NO sourceHeight:16.0f];
...
//    indicator.position = CGPointMake(title.position.x + title.frame.size.width + 8, indicator.position.y);
}
