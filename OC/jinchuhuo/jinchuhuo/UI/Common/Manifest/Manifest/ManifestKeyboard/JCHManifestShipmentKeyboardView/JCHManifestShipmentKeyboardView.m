//
//  JCHManifestShipmentKeyboardView.m
//  jinchuhuo
//
...
@implementation JCHManifestShipmentKeyboardView
...
- (void)layoutSubviews
{
    ...
    CGFloat categoryButtonWidth = self.frame.size.width / 3;
    ...
    [discountButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(topTitleLabel.mas_bottom);
        make.left.equalTo(priceButton.mas_right);
        make.width.mas_equalTo(categoryButtonWidth);
        make.height.mas_equalTo(categoryButtonHeight);
    }];
...
- (void)setButtonTitle:(UIButton *)theButton topTitle:(NSString *)topTitle bottomValue:(NSString *)bottomValue
{
    ...
    if (theButton == discountButton)
    {
        ...
        discount = [[[NSAttributedString alloc] initWithString:@"折"
                                                   attributes:@{NSParagraphStyleAttributeName:paragraphStyle,
                                                                NSForegroundColorAttributeName:valueColor,
                                                                NSFontAttributeName:valueFont}] autorelease];
