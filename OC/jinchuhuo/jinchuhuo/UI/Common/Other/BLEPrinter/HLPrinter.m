 ...
 *  @param boldMode 是否加粗
 */
- (void)setFontBoldMode:(BOOL)boldMode
{
    Byte boldFontModeBytes[] = {0x1B,0x45,boldMode};
    [_printerData appendBytes:boldFontModeBytes length:sizeof(boldFontModeBytes)];
}
 ...
 *  设置二维码类型
 *
 *  @param type   0:PDF417, 1:DataMatrix, 2:QRCODE
 */
- (void)setQRType:(NSInteger)type
{
    Byte codeType [] = {0x1D,0x5A,type};
    [_printerData appendBytes:codeType length:sizeof(codeType)];
}
 ...
 *  @param string 打印的内容
 */
- (void)printQRCode:(NSString *)info size:(CGFloat)size
{
    NSInteger m = 0;        // 指定字符版本
    NSInteger n = 2;        // 纠错等级
    NSInteger k = size;     // 长宽比
    
    NSInteger kLength = info.length + 3; // 数据长度
    NSInteger pL = kLength % 256;   // 低位字节
    NSInteger pH = kLength / 256;   // 高位字节
    
    Byte printQRType [] = {0x1B,0x5A,m,n,k,pL,pH};
    [_printerData appendBytes:printQRType length:sizeof(printQRType)];
    [_printerData appendData:[info dataUsingEncoding:NSUTF8StringEncoding]];
}
...
{
    [self appendQRCodeWithInfo:info size:size alignment:HLTextAlignmentCenter qrCodeCommandType:HLQRCodeCommandTypeOne];
}

- (void)appendQRCodeWithInfo:(NSString *)info size:(NSInteger)size alignment:(HLTextAlignment)alignment  qrCodeCommandType:(HLQRCodeCommandType)commandType
{
    [self setAlignment:alignment];
    if (commandType == HLQRCodeCommandTypeOne) {
        [self setQRCodeSize:size];
        [self setQRCodeErrorCorrection:48];
        [self setQRCodeInfo:info];
        [self printStoredQRData];
        [self appendNewLine];
    } else if (commandType == HLQRCodeCommandTypeTwo) {
        [self setQRType:2];
        [self printQRCode:info size:size];
        [self appendNewLine];
    }
}
